...
import com.weylan.blog.config.value.GlobalValueSetting;
...
public class UserController {
    ...
    @PostMapping("/onLogin")
    public Res userLogin(@RequestBody LoginReq loginReq) throws IOException {
        ...
        if (StringUtils.isNotBlank(code)) {
            ...
            try {
                ...
                if (loginRes == null) {
                    return new Res().error("登录失败");
                }
                return new Res<>().success(loginRes);
            } catch (Exception e) {
                log.error(e.getMessage(), e);
            }
        }
        return new Res().error("code 为空");
    }

    public Res getUserDetail() {
        return null;
    }

    public Res getUserInfoByToken(@RequestHeader("s-token") String token){
